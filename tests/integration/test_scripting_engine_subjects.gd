...
class TestSubjectBoardseek:
	...
	# warning-ignore:unused_variable
		...
		assert_eq(target.card_rotation, 180,
				"Card checked board matching property should be rotated 90 degrees")
		assert_eq(target2.card_rotation, 90,
				"Card checked board matching property should be rotated 180 degrees")
...
class TestSubjectTutor:
	...
	func test_subject_tutor():
		...
		assert_eq("Red",target.properties["Type"],
				"Card of the correct type should be placed checked the board")
		card.scripts = {"manual": {"hand": [
				{"name": "move_card_to_board",
				"subject": "tutor",
				"src_container": "deck",
		...
		assert_eq("Multiple Choices Test Card",target.canonical_name,
				"Card of the correct name should be placed checked the board")
