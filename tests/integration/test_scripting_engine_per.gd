...
class TestPerToken:
	...
	# warning-ignore:return_value_discarded
		...
		assert_not_null(bio_token,
			"Put 1 Bio token per void token checked this card")
		if bio_token:
		...
		assert_eq(hand.get_card_count(), 9,
				"Draw 1 card per void token checked this card")
...
class TestPerBoardseek:
	...
	func test_per_boardseek():
		await table_move(cards[1], Vector2(100,200)).completed
		await table_move(cards[2], Vector2(300,200)).completed
		await table_move(cards[3], Vector2(500,200)).completed
		await table_move(cards[4], Vector2(700,200)).completed
		...
		assert_eq(hand.get_card_count(), 3,
				"Draw 1 card per 0-cost card checked board")
...
class TestFilterPerBoardseek:
	...
	func test_filter_per_boardseek():
		...
		await table_move(cards[3], Vector2(500,200)).completed
		await table_move(cards[4], Vector2(700,200)).completed
		# Flip the card facedown if there's 3 cards checked board
		card.scripts = {"manual": {
				"hand": [
					{"name": "flip_card",
					"subject": "self",
		...
		# Flip the card facedown if there's 4 cards checked board
		card.scripts = {"manual": {
				"hand": [
					{"name": "flip_card",
					"subject": "self",
...
class TestOriginalPrevious:
	...
	func test_original_previous():
		await table_move(card, Vector2(100,200)).completed
		await table_move(target, Vector2(300,200)).completed
		await yield_for(0.1).YIELD
		card.scripts = {"manual": {
			"board": [
				{
	...
	# warning-ignore:unused_variable
		...
		assert_not_null(bio_token_target,
			"Put 5 Bio tokens checked target")
		assert_not_null(blood_token_card,
			"Put 5 Blood tokens checked card")
		if bio_token_target:
