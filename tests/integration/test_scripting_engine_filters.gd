...
class TestFilterGtGeLtLe:
	...
	func test_property_filter_gt_ge_lt_le():
		...
		await yield_to(cards[3]._tween, "tween_all_completed", 0.2).YIELD
		...
		card.scripts = {"manual": {"hand": [
				...
				"degrees": 270}]}}
		card.execute_scripts()
		await yield_to(cards[1]._tween, "tween_all_completed", 0.2).YIELD
...
class TestAnd:
	...
	func test_and():
		...
		await yield_to(target._tween, "tween_all_completed", 0.2).YIELD
	...
	func test_or():
		...
		card.scripts = {"manual": {"hand": [
				...
				"set_faceup": false}]}}
		card.execute_scripts()
		await yield_to(target._flip_tween, "tween_all_completed", 0.4).YIELD
...
class TestStateFilterRotation:
	...
	func test_state_filter_rotation():
		...
		assert_eq(cards[1].card_rotation, 90,
				"Card checked board matching rotation state should be rotated 90 degrees")
	...
	func test_state_filter_faceup():
		...
		await yield_to(target._tween, "tween_all_completed", 0.2).YIELD
		assert_eq(cards[1].card_rotation, 90,
				"Card checked board matching flip state should be rotated 90 degrees")
		assert_eq(cards[2].card_rotation, 90,
				"Card checked board matching flip state should be rotated 90 degrees")
...
class TestFilterTokens:
	...
	func test_state_filter_tokens():
		...
		await yield_to(cards[1]._tween, "tween_all_completed", 0.2).YIELD
		assert_eq(cards[1].card_rotation, 90,
				"Card checked board matching token name rotated 90 degrees")
		assert_eq(cards[2].card_rotation, 90,
				"Card checked board matching token name rotated 90 degrees")
		cards[2].tokens.mod_token("void",-1)
		card.scripts = {"manual": {"hand": [
		...
		await yield_to(cards[1]._tween, "tween_all_completed", 0.2).YIELD
		assert_eq(cards[1].card_rotation, 90,
				"Card checked board not matching token count stays 90 degrees")
		assert_eq(cards[2].card_rotation, 0,
				"Card checked board matching token name rotated 0 degrees")
		...
		await yield_to(cards[1]._tween, "tween_all_completed", 0.2).YIELD
		assert_eq(cards[1].card_rotation, 90,
				"Card checked board not matching tokens stays 90 degrees")
		assert_eq(cards[2].card_rotation, 270,
				"Card checked board matching token name rotated 270 degrees")
...
class TestPerCounter:
	...
	func test_per_counter():
		...
		card.execute_scripts()
		cards[1].execute_scripts()
		await yield_to(card._flip_tween, "tween_all_completed", 0.4).YIELD
