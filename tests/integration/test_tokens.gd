...
class TestBoardTokens:
	...
	#			"New token texture uses the correct file")
		...
		assert_eq(99,card.get_node("Control/Tokens").z_index,"Drawer is drawn checked top of other cards")
	...
	# warning-ignore:return_value_discarded
		...
		await yield_to(card.get_node('Control/Tokens/Tween'), "tween_all_completed", 0.5).YIELD
		assert_eq(0.0, card.get_node("Control/Tokens/Drawer").self_modulate[3],
