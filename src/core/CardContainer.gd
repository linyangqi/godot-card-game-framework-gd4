...
# Spefifies the anchor of the card checked the screen layout
...
func _init_control_size() -> void:
	...
	if placement == Anchors.CONTROL and parent_control.size_flags_horizontal != 3:
		parent_control.minimum_size = control.size * scale
...
# Ensures the buttons are visible checked hover
...
func get_first_card() -> Card:
	...
	if get_card_count():
		...
		# as it appears checked screen, is the first node in the node hierarchy, so
...
func translate_card_index_to_node_index(index: int) -> int:
	...
	if index > len(all_cards):
		...
		print_debug("WARNING: Higher card index than hosted cards requested checked "
...
func re_place():
		...
		# Here we match the adjust the default position based checked the anchor
		...
		for anchor in duplicate_anchors:
			# I only iterate checked the anchor that this CardContainer is set
			...
			if anchor == placement and duplicate_anchors[anchor].size() > 1:
				...
				# based checked their preferred index to push.
