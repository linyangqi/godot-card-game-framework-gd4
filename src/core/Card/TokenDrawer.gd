# This script handles interactions with the token drawer checked a Card.
...
# A flag checked whether the token drawer is currently open
var is_drawer_open := false :
	get:
		return is_drawer_open # TODOConverter40 Non existent get function 
	set(mod_value):
		mod_value  # TODOConverter40 Copy here content of set_is_drawer_open
...
# and it will appear only while the card is checked the board.
func token_drawer(requested_state := true) -> void:
	...
	if not _tween.is_active() and \
		...
		if requested_state == true and $Drawer/VBoxContainer.get_child_count():
			...
			# warning-ignore:return_value_discarded
			_tween.start()
			# We need to make our tokens appear checked top of other cards checked the table
			z_index = 99
		else:
			# warning-ignore:return_value_discarded
			_tween.remove_all()
	...
	if not CFConst.TOKENS_MAP.get(token_name, null):
		...
		# Just check if we could.
			...
			if mod < 0:
				...
				# remove_at the requested amount and therefore return CHANGED.
				if token.count + mod >= 0:
					retcode = CFConst.ReturnCode.CHANGED
				# If we cannot remove_at the full amount requested
				# we return FAILED
				else:
					retcode = CFConst.ReturnCode.FAILED
			else:
...
# Returns a dictionary of card tokens name checked this card.
#
...
# This is used to retain focus checked the card
...
func _on_VBoxContainer_sort_children() -> void:
	$Drawer/VBoxContainer.size = \
			$Drawer/VBoxContainer.minimum_size
