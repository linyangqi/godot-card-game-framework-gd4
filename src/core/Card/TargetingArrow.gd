...
# Will generate a targeting arrow checked the card which will follow the mouse cursor.
...
func _on_ArrowHead_area_exited(area: Area2D) -> void:
	if area.get_class() == 'Card' and area in _potential_targets:
		# We remove_at the card we stopped hovering from the _potential_targets
...
func _draw_targeting_arrow() -> void:
	# This variable calculates the card center's position checked the whole board
	var card_half_size
	...
	# but we offset it by the position of the card center checked the map
...
#		curve.add_point($TargetLine.to_local(offmid), Vector2(0,0), Vector2(0,0))
	...
	# the last point checked the line towards the mouse position
