...
func _on_Delete_pressed() -> void:
	...
	if dir.dir_exists(CFConst.DECKS_PATH):
		var op_result = dir.remove_at(CFConst.DECKS_PATH + _deck_name.text + '.json')
		if op_result == OK:
			_set_notice("Deck deleted from disk. Current list not cleared")
		elif op_result == FAILED:
			_set_notice("Deck not found checked disk", Color(1,1,0))
		elif op_result == ERR_FILE_NO_PERMISSION:
			_set_notice("Permission Denied", Color(1,0,0))


# Generates a random deck name using checked the deck_name_randomizer
