...
func _ready() -> void:
	if property_width_exceptions.has(CardConfig.SCENE_PROPERTY):
		_card_type_header.minimum_size.x = property_width_exceptions[CardConfig.SCENE_PROPERTY]
	else:
		_card_type_header.minimum_size.x = default_property_width
	...
	for p in card_props:
		...
		if not property.begins_with('_')\
			...
			if property_width_exceptions.has(property):
				new_label.minimum_size.x = property_width_exceptions[property]
			else:
				new_label.minimum_size.x = default_property_width
			new_label.minimum_size.y = 20
