...
func setup(_card_name: String) -> void:
	...
	if card_viewer.property_width_exceptions.has(CardConfig.SCENE_PROPERTY):
		_card_type.minimum_size.x = card_viewer.property_width_exceptions[CardConfig.SCENE_PROPERTY]
	else:
		_card_type.minimum_size.x = card_viewer.default_property_width
	for p in card_properties:
		...
		if not property.begins_with('_')\
			...
			if card_viewer.property_width_exceptions.has(property):
				new_label.minimum_size.x = card_viewer.property_width_exceptions[property]
			else:
				new_label.minimum_size.x = card_viewer.default_property_width
