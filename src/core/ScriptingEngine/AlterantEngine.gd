...
#
# The automation is based checked [ScriptAlter] definitions. 
#
# This class is checked request when one of a number of specific tasks
...
var alteration := 0
# In case the alteration is dependent checked a subject, we store the one we're checking against here.
var subject
...
# it calculates how much to alter, based checked the script definition.
