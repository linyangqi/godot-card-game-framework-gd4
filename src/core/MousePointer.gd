...
# As we cannot rely checked Control node relevant signals due to Godot bugs
# such as:
...
var is_disabled := false :
	get:
		return is_disabled # TODOConverter40 Non existent get function 
	set(mod_value):
...
func _on_MousePointer_area_exited(area: Area2D) -> void:
	if not is_disabled:
		# We stop the highlight checked any areas we exit with the mouse.
...
func determine_global_mouse_pos() -> Vector2:
	...
	#var scaling_offset = get_tree().get_root().get_node('Main').get_viewport().get_size_2d_override() * OS.window_size
...
func disable() -> void:
	...
	for area in overlaps:
		# We stop the highlight checked any areas we were currently highlighting
		if area as Card or area as CardContainer:
			area.highlight.set_highlight(false)
		elif area.get_parent() as BoardPlacementSlot:
			area.get_parent().set_highlight(false)
...
# Parses all collided objects and figures out which card, if any, to focus checked and
...
func _discover_focus() -> void:
	...
	for area in overlaps:
		if area as Card:
			...
			# dragged card
			# and the checked area has to be checked the board
			if current_focused_card \
...
#			print_debug(potential_cards)
	...
	# Dragging onto cards, takes priority over board placement grid slots
		...
		# We sort the potential cards by their index checked the board
		potential_cards.sort_custom(Callable(CFUtils,"sort_index_ascending"))
		# The candidate always has the highest index as it's drawn checked top of
		# others.
	...
	# if not, then we remove_at focus.
	#
	...
	if not cfc.card_drag_ongoing:
		...
		if cfc.card_drag_ongoing.attachment_mode != Card.AttachmentMode.DO_NOT_ATTACH \
			...
			# a game specifies not all cards are potential hosts.
				...
				is_valid = false
		# If a card is only allowed to be placed checked specific grids,
		...
		if cfc.card_drag_ongoing.board_placement in [
			...
			if slot.get_grid_name() != cfc.card_drag_ongoing.mandatory_grid_name:
				is_valid = false
		# If a card is not allowed checked the board
