...
#		pass
##		card_focus.minimum_size = _current_focus_source.canonical_size * _current_focus_source.focused_scale * cfc.curr_scale
#		card_focus.minimum_size.y *= 1.25
...
#		focus_info.size.x = _current_focus_source.canonical_size.x * _current_focus_source.focused_scale * cfc.curr_scale
	# The below makes sure to display the closeup of the card, only checked the side
	...
	if _current_focus_source and is_instance_valid(_current_focus_source)\
		...
		$VBC.position.y = 0
	# The below performs some garbage collection checked previously focused cards.
...
func focus_card(card: Card, show_preview := true) -> void:
	# We check if we're already focused checked this card, to avoid making duplicates
	# the whole time
	if not _current_focus_source:
		# We make a duplicate of the card to display and add it checked its own in
		# our viewport world
		# This way we can standardize its scale and look and not worry about
		# what happens checked the table.
		var dupe_focus: Card
		...
		# We make the viewport camera focus checked it
		...
		# To ensure they are shown as expected checked the screen
