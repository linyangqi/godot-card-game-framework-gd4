...
func _ready() -> void:
	...
	# instead of defining them checked the scene.
...
# You can remove_at this function and the FancyMovementToggle button
# without issues
...
func load_test_cards(gut := true) -> void:
	var extras = 11
	# Hardcoded the card order because for some reason, GUT checked low-powered VMs
