...
# Since we have a label checked top of our image, we use this to scale the label
# as well
...
func scale_to(scale_multiplier: float) -> void:	
	if font_scaled != scale_multiplier:
		...
		label.minimum_size *= scale_multiplier
		# We need to adjust the Viewed Container
		# a bit more to make the text land in the middle
#		$"VBoxContainer/CenterContainer".minimum_size *= scale_multiplier * 1.5
